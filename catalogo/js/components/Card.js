...
// Cria um cartão de filme/série na página.
export function createCard(item) {
    const card = document.createElement('div');
    card.className = 'movie-card';
    if (item.progress) {
        card.classList.add('has-progress'); // Marca o cartão se tiver progresso.
    }
    ...
    const videoId = getYouTubeId(item.youtube); // Pega o ID do vídeo.

    card.appendChild(iframe);
    card.appendChild(img);

    const ageBadge = getRandomAgeBadge(); // Selo de idade aleatório.
    ...
    // Se existir progresso, adiciona barra de progresso ao cartão.
    if (item.progress) {
    ...
    card.addEventListener('mouseleave', () => {
        clearTimeout(playTimeout); // Cancela o vídeo se o mouse sair rápido.
